    ...
    private CameraHolder() {
		...
		if(androidVersion < 10){
			...
			for (int i = 0; i < mNumberOfCameras; i++) {
				...
				CameraInfo cameraInfo = (CameraInfo)mInfo[i];
				android.hardware.Camera.getCameraInfo(i,cameraInfo);
				if (mBackCameraId == -1 && cameraInfo.facing == CameraInfo.CAMERA_FACING_BACK) {
					mBackCameraId = i;
				}
				if (mFrontCameraId == -1 && cameraInfo.facing == CameraInfo.CAMERA_FACING_FRONT) {
					mFrontCameraId = i;
				}
			}
		}
